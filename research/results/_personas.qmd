...
### Clustering College Students to Build Personas
...
Segmentation helps design features for groups of people with shared needs. The Likert scale is a psychometric scale developed by Rensis Likert already in the 1930s, and it's commonly used to this day in questionnaires to measure respondents' attitudes, opinions, and perceptions [@sullivanAnalyzingInterpretingData2013]. K-Means, K-Modes, and K-Prototypes are unsupervised machine learning algorithms, used for finding patterns in the data. Here I've opted to use K-Means and Principal Component Analysis (PCA), which is used to convert data to lower dimension space (this is a simpler predecessor of *embeddings* used in large-language models).
...
| Agreement Between Personas | Design Implications |
|-------------------------|------------------------------------------|
| **Health & Safety (4.20)** | Highest average means students *actively* engage with personal well-being. Nearly everyone agrees, so messaging can be broad. |
| **Climate & Pollution (4.00)** | Strong support for big-picture environmental stakes. Scores cluster only a bit lower than #1, but difference isn’t huge. |
| **Env. Awareness & Action (3.80)** | Students *like* the idea of acting green, but concrete effort lowers the mean a bit. |
| **Personal Finance & Investment (3.50)** | Variance is wider; a sub-segment rates it high, others are less engaged. |
| **Ethical Consumption & Labor (3.20)** | Agreement exists but real people far away feel abstract. Spotlighting specific worker stories, may help this score to climb. |
| **Tech & AI Engagement (3.10)** | Curiosity tempered by caution. The topic is newer, so opinions are still forming. |
| **Economic Growth vs Sustainability (2.90)** | Lowest mean = hardest sell; “trade-off” framing pushes students to disagree. |

In summary, *Safety \> Planet \> Personal Eco-action*. Students first protect themselves (food safety, health), then the planet, then think about their habits. Money talk sits mid-tier. Finance topics aren’t ignored (3.5 / 5) but aren’t hype either. Ethical labor and AI still feel abstract. Sweat-shop worries and AI curiosity land in the middle. Economic trade-offs are the hardest sell. When sustainability sounds like “sacrifice growth,” support drops (\< 3).

> *Overall Design Implications*:
>
> For the widest support between all personas, frame eco-features around personal health wins to boost engagement. Link sustainable investing to tangible health or climate outcomes to lift enthusiasm. Communications angle: Storytelling about real human impact behind labor issues and show AI as a practical helper, not sci-fi. Policy framing: Highlight win-win scenarios (green jobs, savings) instead of “growth vs. green” rhetoric.
